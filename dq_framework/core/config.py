...
class GEConfig(BaseSettings):
    ...
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        self.data_context_root_dir = os.getenv(
            "GE_DATA_CONTEXT_ROOT_DIR", self.data_context_root_dir
        )
        self.expectations_store_name = os.getenv(
            "GE_EXPECTATIONS_STORE_NAME", self.expectations_store_name
        )
        self.validations_store_name = os.getenv(
            "GE_VALIDATIONS_STORE_NAME", self.validations_store_name
        )
        self.evaluation_parameter_store_name = os.getenv(
            "GE_EVALUATION_PARAMETER_STORE_NAME",
